...
export function NodeChip({ nodeId }: Props) {
  ...
  if (!node) {
    return (
      <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full border border-mist text-[10.5px] text-stone">
        unknown
      </span>
    )
  }
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full border border-mist hover:border-graphite transition-colors text-[10.5px] text-ink"
    >
